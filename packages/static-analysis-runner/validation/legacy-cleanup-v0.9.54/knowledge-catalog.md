...
- Fingerprint: `8a584d35c2f1023028ef2755f8d6d67aefbaf21d666ba500e05372bd8bdc4533`
...
- Можно выбрать в knowledge_profile/v2: **да**
- Готовность: `current_typed` — Materialization зарегистрирована в общем KLC runtime и исполняется через типизированный контракт.
...
- Обязательные источники:
  - **Observed Java storage usage evidence** — `storage-usage-evidence`; producer/analyzer: `java-storage-usage-analyzer`; статус: `registered`
...
- Обязательные источники:
  - **Repository SQL analysis evidence** — `sql-analysis`; producer/analyzer: `sql-analysis-analyzer`; статус: `registered`
- Дополнительные источники:
  - **Предоставленная физическая модель** — `physical-model`; producer/analyzer: `внешний/не зарегистрирован`; статус: `not_applicable`
...
Детерминированная композиция опубликованных SQL-знаний нескольких репозиториев без повторного анализа исходников.

- Области: `workspace`
- Можно выбрать в knowledge_profile/v2: **да**
- Готовность: `current_typed` — Materialization зарегистрирована в общем KLC runtime и исполняется через типизированный контракт.
- KLC materialization: `workspace-sql-catalog`
- В базу знаний войдут: общий SQL-инвентарь workspace; источники и назначения по репозиториям; поля и роли использования; расчёты и происхождение полей; target lineage, candidates и контекст добавления атрибута; покрытие и пробелы по репозиториям
- Обязательные источники:
- Обязательные модели KLC:
  - `sql-observed-data-usage` из `sql-analysis`
