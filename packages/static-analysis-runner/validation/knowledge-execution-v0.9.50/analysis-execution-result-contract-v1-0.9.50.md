...
- Current manifest variants: **6**
- Fully compliant variants: **3**
- Variants with any typed artifact registry: **6**
...
Scope: `knowledge_execution_plan`. Target compliance: `full`.
...
Runner now has one canonical product runtime: knowledge_execution_plan/v1 drives Core evidence execution, generic typed-artifact registration, KLC materialization dependencies and capability publication without Task, Suite, Core Profile or knowledge-family-specific dispatch.
...
2. **`generic_core_evidence_runtime_and_executor`** — Runner registers arbitrary typed Core artifacts without evidence-family branches.
3. **`knowledge_execution_plan/v1`** — Knowledge Profile, actual inputs and producer catalogs compile into one validated DAG.
4. **`knowledge_execute_and_result/v1`** — The product runtime executes the DAG directly and emits one canonical result.
5. **`next_independent_evidence_and_materializer`** — Prove that a new evidence family requires no production-code change in Runner.
6. **`switch_consumers_to_generic_knowledge_execution`** — Consumers should switch only after the second evidence family proves the runtime boundary.

## Explicitly deferred

- Knowledge API, Reporting and Assistant migration until the second evidence family is proven
